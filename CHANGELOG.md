...
## [3.4.4] - 2024-01-28 - "Enhanced Detection"

### 🎯 主要更新
优化 Claude Code 检测机制，增强配置管理功能，提供更好的用户体验。

### ✨ 新功能

#### 1. Claude Code 检测优化
- 修复 Mac 版本无法正确检测 Claude Code 的问题
- 通过 npm 包列表智能检测 `@anthropic-ai/claude-code`
- 支持多路径自动搜索和版本识别
- 替换已废弃的 `npm bin -g` 命令为 `npm config get prefix`

#### 2. 配置管理增强
- **保存并启动配置**：一键保存配置并直接启动终端
- **仅保存**：新增独立的保存按钮，只保存配置不启动
- **还原官方配置**：快速还原 Claude Code 官方默认配置
